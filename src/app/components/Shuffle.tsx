...
const Shuffle = ({
  ...
  const hoverHandlerRef = useRef<((this: HTMLElement, ev: Event) => any) | null>(null);

  useEffect(() => {
    // Wait for font loading so char measurement (width/height) is accurate.
  ...
  useEffect(() => {
    ...
    const removeHover = () => {
      if (hoverHandlerRef.current && ref.current) {
        ref.current.removeEventListener("mouseenter", hoverHandlerRef.current as any);
    ...
    const build = () => {
      ...
      chars.forEach((ch) => {
        ...
        Object.assign(inner.style, {
          display:
            shuffleDirection === "up" || shuffleDirection === "down" ? "inline-block" : "inline-block",
          whiteSpace:
    ...
    const inners = () => wrappersRef.current.map((w) => w.firstElementChild as HTMLElement | null).filter(Boolean) as HTMLElement[];
